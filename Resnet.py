...
def save_checkpoint(state, epoch, filename='res_checkpoint'):
	print ("=> Saving model")
...
# resnet.fc = nn.Linear(n_ftrs*100, 228)
resnet.fc = nn.Sequential(nn.Linear(n_ftrs*100, 228),nn.Sigmoid())

if use_gpu:
	resnet = resnet.cuda()

optimizer = torch.optim.Adam(resnet.fc.parameters(),lr=1e-6)


print('RESNET')
for epoch in range(1, num_epoch + 1):
	start = time.time()
	train(epoch, resnet, optimizer, train_loader)
	end = time.time()
	print('Training Time: {} minutes'.format((end - start)/60))

	validation(resnet, validation_loader)
